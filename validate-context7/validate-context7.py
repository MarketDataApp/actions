...
Usage:
    validate-context7.py [PATH] [--fail-if-missing]

PATH defaults to ./context7.json. A missing file is reported and passes, so the
check is safe to add to a repository before it adopts a config; pass
--fail-if-missing to require one.
...
def main(argv: list[str]) -> int:
    args = argv[1:]
    fail_if_missing = "--fail-if-missing" in args
    positional = [a for a in args if not a.startswith("--")]
    path = positional[0] if positional else "context7.json"

    if not os.path.exists(path):
        if fail_if_missing:
            annotate(path, "no context7.json found, and --fail-if-missing was set")
            return 1
